...
def main():
    ...
    if data_args.line_by_line:
        ...
        def tokenize_function(examples):
            ...
            # mask specific token
            examples[text_column_name] = [examples[text_column_name][i].replace('<MASK>', MASK_TOKEN) for i in range(len(examples['target']))]
            ...
            for i in range(len(examples[text_column_name])):
                ...
                if MASK_TOKEN_ID in tokenized_data.input_ids[i]:
                    label_arr[tokenized_data.input_ids[i].index(MASK_TOKEN_ID)] = tokenizer.convert_tokens_to_ids(examples['target'][i])
